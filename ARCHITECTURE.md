...
| [0041](docs/adr/0041-scheduling-gates-and-bounded-auto.md) | Pre-scheduling gates closed (profile-staleness typed refusal before prepare(); send-confirmation as positive-observation-only port with no send capability) and the bounded `auto` pass that structurally cannot confirm or submit -- human-gated submission is permanent |
